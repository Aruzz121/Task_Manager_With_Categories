# Instrucciones — Task Manager with Categories
...
- **Controladores**:
  - `TasksController`: `GET /api/tasks` (con filtros opcionales e independientes por `categoryId` y `status`), `POST /api/tasks`, `PUT /api/tasks/{id}` (editar título/descripción/categoría), `PATCH /api/tasks/{id}/toggle-status` (marcar pendiente/completada) y `DELETE /api/tasks/{id}`.
...
- En cada tarea: casilla para **marcarla como completada**, ícono de **lápiz para editarla** (título, descripción y/o categoría) e ícono de **bote de basura para eliminarla**.
